interface LogoProps {
  size?: number;
  className?: string;
}

/**
 * Marca Valore — V aninhado, paths extraídos do logo oficial.
 * viewBox original 1536x1024 preservado para fidelidade total ao desenho.
 */
export function ValoreMark({ size = 32, className = "" }: LogoProps) {
  const height = size * (1024 / 1536);
  return (
    <svg
      width={size}
      height={height}
      viewBox="0 0 1536 1024"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
    >
      <g fill="currentColor">
        <path d="M725.8 810.5 c-9.6 -2.4 -17.7 -7.1 -25.3 -14.9 -5.8 -5.9 -9.4 -11.2 -20.9 -31.1 -31 -53.4 -69.4 -118.2 -174.1 -293 -142.5 -237.9 -134.1 -223.7 -135.5 -230 -2.2 -10 2.7 -20.2 12.2 -25.2 l5.3 -2.8 125.5 0 c118.6 0 125.9 0.1 132 1.8 13.6 3.9 27.6 12.6 35.7 22.3 2.2 2.7 8.3 11.9 13.5 20.4 5.2 8.5 16.7 27.2 25.5 41.5 8.7 14.3 23.3 38 32.3 52.7 9 14.8 16.7 26.8 17.1 26.8 0.4 0 2.4 -2.8 4.5 -6.3 20 -33.3 78.5 -126.8 83.4 -133.2 5.9 -7.8 14.6 -14.7 24.5 -19.5 15.1 -7.3 9.4 -7 143.3 -7 136.2 0 127.1 -0.5 135.4 8 3.6 3.8 4.9 6 5.9 10.4 2.3 10.5 4 7.1 -41.9 84.6 -8 13.5 -21.7 36.7 -30.5 51.5 -8.8 14.9 -23.5 39.6 -32.7 55 -53.7 89.9 -120.1 202.1 -179 302.5 -13.4 22.8 -27.4 46.7 -31.1 53.1 -10.7 18.1 -18.1 25.2 -32.5 30.7 l-6.9 2.7 -40 0.2 c-31.7 0.2 -41.2 -0.1 -45.7 -1.2z m83 -33.1 c5.5 -2.2 9.7 -6.8 15.6 -16.9 2.9 -4.9 14.4 -24.7 25.6 -44 35.7 -61.5 98.6 -168 182.5 -309 12.8 -21.4 39.4 -66.3 59.3 -99.8 l36 -60.7 -113.7 0.2 -113.6 0.3 -6.5 3.2 c-10.9 5.4 -7.5 0.4 -64.7 93.3 -13.7 22.3 -28.5 46.4 -32.8 53.5 -10.1 16.7 -15.1 21.8 -22.6 23.5 -4.7 1.1 -6.4 1 -11.1 -0.4 -7.3 -2.1 -10.8 -5.8 -21.4 -23.1 -4.8 -7.7 -12.9 -21 -18.2 -29.5 -10.5 -17.2 -43.6 -71.4 -57.5 -94.3 -9.4 -15.6 -12.6 -19 -21.7 -23.5 l-5.5 -2.7 -114.9 -0.3 -114.9 -0.2 1.5 2.2 c1.8 2.9 98.3 163.8 151.8 253.3 40 66.9 119.9 202.5 153.8 261 4.6 7.9 7.1 10.6 12.8 13.5 3.7 1.8 6.1 1.9 40.2 2 30.6 0 36.8 -0.2 40 -1.6z" />
        <path d="M747.8 724.7 c-8.1 -2.8 -16.5 -8.4 -21.1 -14 -1.8 -2.3 -27.6 -45.1 -57.2 -95.2 -29.6 -50 -56.4 -95.3 -59.5 -100.5 -3.1 -5.2 -9.4 -15.8 -14 -23.5 -4.5 -7.7 -26.7 -44.8 -49.3 -82.5 -22.5 -37.6 -41.6 -70 -42.4 -71.9 -2.3 -5.3 -2 -13.8 0.5 -18.7 2.8 -5.5 9.3 -10.9 14.7 -12.3 2.9 -0.7 21.2 -1.1 54.2 -1.1 54.3 0 56.2 0.2 66.7 5.8 2.8 1.5 7.3 5 10 7.7 5 5.1 9 11.7 87.6 144.7 16.8 28.4 30.8 51.4 31.1 51 0.8 -0.8 23.2 -38.7 59.9 -101.2 15 -25.6 31.4 -53.5 36.5 -62 5.1 -8.5 11.1 -18.7 13.3 -22.7 6.6 -11.4 17.6 -19.6 30 -22.2 7.5 -1.6 102.6 -1.5 109 0.1 3.4 0.8 6 2.5 9.8 6.2 5.8 5.8 7.9 11.9 6.8 19.2 -0.7 5.1 0 3.8 -39.4 69.9 -30.1 50.7 -55.3 93.1 -70 118 -4.2 7.2 -17.1 29 -28.7 48.5 -11.5 19.5 -34.8 58.9 -51.8 87.5 -32.6 55 -36.3 60.4 -46.5 65.9 -7.8 4.2 -15.1 5.6 -30 5.6 -11.4 0 -14.5 -0.4 -20.2 -2.3z m31.8 -31.2 c2.8 -0.8 6 -2.5 7.1 -3.7 1.2 -1.3 7.1 -10.6 13.1 -20.8 5.9 -10.2 18.7 -31.8 28.4 -48 22 -37.1 63.7 -107.4 83.8 -141.5 8.4 -14.3 18.6 -31.4 22.5 -38 3.9 -6.6 13.2 -22.1 20.5 -34.5 7.3 -12.4 17.2 -29 22 -37 4.8 -8 11 -18.4 13.8 -23.2 l5.2 -8.8 -40.1 0 c-42.8 0 -44.1 0.1 -48.5 5.1 -1.2 1.3 -9.2 14.6 -17.9 29.4 -8.7 14.9 -20 34 -25 42.5 -5.1 8.5 -12.5 21.1 -16.5 28 -20.1 34.4 -59.5 100.8 -61.2 103.2 -4.6 6.5 -16 10.2 -24.4 7.9 -5.8 -1.5 -12.7 -8 -17.6 -16.4 -4.6 -8.1 -18 -30.8 -44.3 -75.2 -9.8 -16.5 -25.8 -43.5 -35.5 -60 -32.5 -55 -35.5 -59.8 -38.3 -61.9 -2.8 -2 -4 -2.1 -43.8 -2.4 -22.5 -0.1 -40.9 -0.1 -40.9 0.2 0 0.2 17 28.9 37.9 63.8 20.8 34.8 47.3 79.3 58.8 98.8 17.9 30.3 51 86.2 100 168.7 11.6 19.6 14.4 22.8 20.7 24.3 5.3 1.3 15 1.1 20.2 -0.5z" />
      </g>
    </svg>
  );
}

export function ValoreLogo({ size = 28, className = "" }: LogoProps) {
  return (
    <div className={`flex items-end gap-2 ${className}`}>
      <ValoreMark size={size} className="shrink-0" />
      <span
        className="font-display font-bold tracking-tight leading-none"
        style={{ fontSize: size * 0.62 }}
      >
        Valore
      </span>
    </div>
  );
}
